...
export const Link = forwardRef(function RefLink({ nav, to, children, ...props }, ref) {
  // if this is a navigation menu link, we can bail out now.
  if (nav) {
    return <ReactNavLink ref={ ref } to={ to } { ...props }>{ children }</ReactNavLink>    
  }
  ...
  const LinkComponent = externalUrlMatch
    ? ExternalLink
      : mailtoMatch
        ? MailtoLink
        : ReactLink
  return <LinkComponent ref={ ref } to={to} { ...props }>{children}</LinkComponent>
})
